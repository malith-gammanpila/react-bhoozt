...
const StyledOkBtn = Styled(StyledCopyBtn)`
...
const ShareFreeGame = (props) => {
	const { height, width } = useWindowDimensions();
	const { user, setCloseAllPopups } = HomeContainer.useContainer();
	...
	const { newGame } = CreateFunContainer.useContainer();
	...
	const handleCreateInvitation = async (gameId) => {
		...
		if (status >= 300) {
			...
			if (newGame && newGame.title) {
				mpSharePvtGame(gameId, newGame.title, 'Failure');
			}
		} else {
			...
			// Mixpanel analytics
			if (newGame && newGame.title) {
				mpSharePvtGame(gameId, newGame.title, 'Success');
			}
		}
	};
			...
			<StyledWrapper>
				...
				{referral && referral.length > 0 ? (
					<StyledContentContainer>
						...
						<StyledOkBtn onClick={handleClickOk}>
							Cancel
						</StyledOkBtn>
					</StyledContentContainer>
				) : (
					<StyledFullContentContainer>
						<StyledCopyToClipboard text={referral}>
							<StyledCreateLinkBtn
								onClick={() =>
									handleCreateInvitation(newGame.id)
								}
								disabled={isBusy}
							>
